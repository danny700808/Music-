...
/* =========================================================
 * LINE 綁定 Webhook 2026-06-09
 * ---------------------------------------------------------
 * 使用者在 LINE 官方帳號輸入：柚子綁定 user@example.com
 * 系統會用「實際送出訊息的 LINE source.userId」綁到該員工主檔。
 * 這可以避免前端或主管暫存資料誤把員工綁到主管手機。
 * ========================================================= */
...
function parseLineBindCommand(text) {
  ...
  const m = raw.match(/^柚子綁定\s+([^\s]+@[^\s]+)$/i);
  if (!m) return null;
  return clean(m[1]).toLowerCase();
}

async function findEmployeeByEmailForLineBind(email) {
  const e = clean(email).toLowerCase();
  if (!e) return null;
  const queries = [
    db.collection('employees').where('email', '==', e).limit(5),
    db.collection('employees').where('Email', '==', e).limit(5),
  ];
  ...
  const scan = await db.collection('employees').limit(500).get();
  let found = null;
  scan.forEach((doc) => {
    if (found) return;
    const data = doc.data() || {};
    const mail = clean(data.email || data.Email || data['Email']).toLowerCase();
    if (mail === e) found = doc;
  });
  return found;
}
...
async function handleLineBindEvent(event) {
  ...
  const email = parseLineBindCommand(text);
  ...
  const doc = await findEmployeeByEmailForLineBind(email);
  if (!doc) {
    await replyLineMessage(replyToken, `綁定失敗：找不到 ${email} 對應的員工資料。請確認 Email 是否和員工系統註冊資料完全相同。`);
    return { handled: true, ok: false, reason: 'employee_not_found', email };
  }
  ...
  await db.collection('lineBindLogs').add({
    employeeDocId: doc.id,
    employeeId: clean(before.employeeId || before['員工ID'] || doc.id),
    name: clean(before.name || before['姓名']),
    email,
    lineUserId,
    sourceType: clean((event.source || {}).type || ''),
    eventType: clean(event.type || ''),
    boundAt: admin.firestore.FieldValue.serverTimestamp(),
    boundAtText: new Date().toISOString(),
  });
  const name = clean(before.name || before['姓名'] || email);
  await replyLineMessage(replyToken, `綁定成功：${name}\n之後會依通知設定接收 LINE 通知。`);
  return { handled: true, ok: true, employeeDocId: doc.id, email };
}
